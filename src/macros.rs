...
macro_rules! arith_forward_round {
    (
        ...
        impl AssignRound<$OwnedIncomplete<'_>> for $Big {
            ...
            fn assign_round(
                &mut self,
                src: $OwnedIncomplete<'_>,
                round: $Round,
            ) -> $Ordering {
                <$Big as AssignRound<&$OwnedIncomplete<'_>>>::assign_round(
                    self, &src, round,
                )
            }
        }
...
macro_rules! arith_noncommut_round {
    (
        ...
        impl AssignRound<$FromOwnedIncomplete<'_>> for $Big {
            ...
            fn assign_round(
                &mut self,
                src: $FromOwnedIncomplete<'_>,
                round: $Round,
            ) -> $Ordering {
                <$Big as AssignRound<&$FromOwnedIncomplete<'_>>>::assign_round(
                    self, &src, round,
                )
            }
        }
