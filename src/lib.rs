...
//!   the left-hand-side operand, which would need to have the same
//!   type.
...
//! 4. For [`Rational`][rug rat] numbers, operations are also
//!    supported for tuples containing two primitives: the first is
//!    the numerator and the second is the denominator which cannot be
//!    zero. The two primitives do not need to have the same type.
//! 5. For [`Complex`][rug com] numbers, operations are also supported
//!    for tuples containing two primitives: the first is the real
//!    part and the second is the imaginary part. The two primitives
//!    do not need to have the same type.
//!
//! ## Operators
//!
...
//! `a >> -5`. The shifting operators are also supported for the
