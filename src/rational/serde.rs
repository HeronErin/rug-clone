...
impl<'de> Visitor<'de> for RationalVisitor {
    ...
    fn visit_seq<V>(self, mut seq: V) -> Result<Rational, V::Error>
    where
        V: SeqAccess<'de>,
    {
        ...
        Rational::from_str_radix(&value, radix)
            .map_err(|e| DeError::custom(format_args!("{}", e)))
    }

    fn visit_map<V>(self, mut map: V) -> Result<Rational, V::Error>
    where
        V: MapAccess<'de>,
    {
        let mut radix = None;
...
impl<'de> Deserialize<'de> for Rational {
    ...
    where
        D: Deserializer<'de>,
    {
        deserializer.deserialize_struct("Rational", FIELDS, RationalVisitor)
    }
}
